...
def preprocess_data(df):
    ...
    # Create 'Is_Open' column based on working hours
    df['Is_Open'] = [
    ...
    # Define transformers
    datetime_transformer = DatetimeFeatures(variables="index", features_to_extract=["hour", "day_of_week", "weekend", "month", "day_of_month"])
    cyclical_transformer = CyclicalFeatures(variables=['hour', 'day_of_week'])
    lag_transformer = LagFeatures(variables=[target], periods=[168, 169, 185, 264, 186, 263, 184, 218, 187, 188])
