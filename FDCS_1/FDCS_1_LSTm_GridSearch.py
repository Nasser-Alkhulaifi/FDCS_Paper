#FDCS_1_LSTM.py
# It is generally not needed to tune the number of epochs because a built-in callback
# is passed to model.fit() to save the model at its best epoch evaluated by the validation_data.
...
def run_lstm_model(X_train_normalized, y_train_normalized, X_val_normalized, y_val_normalized, output_names, look_back=look_back):
    ...
    X_train_val_reshaped, y_train_val_reshaped = create_dataset(X_train_normalized, y_train_normalized, look_back=look_back)
    X_test_reshaped, y_test_reshaped = create_dataset(X_val_normalized, y_val_normalized, look_back=look_back)
    ...
    for i, output_name in enumerate(output_names): 
        ...
        for fold, (train_index, val_index) in enumerate(tscv.split(X_train_val_reshaped)):
            ...
            rmse = sqrt(mse)
            metrics_df = pd.concat([metrics_df, pd.DataFrame({'Fold': [fold+1], 'Set': ['Cross-Validation'], 'Model': ['LSTM'], 'Output': [output_name], 'MAE': [mae], 'RMSE': [rmse]})], ignore_index=True)
        ...
        rmse = sqrt(mse)
        metrics_df = pd.concat([metrics_df, pd.DataFrame({'Fold': ['Final'], 'Set': ['Test'], 'Model': ['LSTM'], 'Output': [output_name], 'MAE': [mae], 'RMSE': [rmse]})], ignore_index=True)
        ...
        pred_true_ValSet = pd.concat([pred_true_ValSet, temp_df], ignore_index=True)
    ...
    relevant_hyperparams = ['units_first_layer', 'num_layers', 'learning_rate'] + [f'units_layer_{i+1}' for i in range(num_layers)] + [f'relu_activation_{i}' for i in range(num_layers) if best_hyperparameters.get(f'relu_activation_{i}')] + [f'dense_units_{i}' for i in range(num_layers) if best_hyperparameters.get(f'relu_activation_{i}')]
