...
def preprocess_data(df):
    ...
    6. Splits the data into features (X) and target (y) variables.
    7. Normalizes the features and target variables using MinMaxScaler.
    8. Saves the target scaler to a file.

    Args:
        df (pd.DataFrame): Input DataFrame containing the data to be preprocessed.

    Returns:
        tuple: Normalized training, validation, and test sets in the form of 
    ...
    lag_transformer = LagFeatures(
        variables=[target],
        periods=[168, 192, 216, 264, 240, 288, 336, 312, 215, 263],
    )
